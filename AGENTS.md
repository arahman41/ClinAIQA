...
PENDING: ONE final held-out re-measurement (user runs `docker compose exec api python -m clinaiqa.eval.report_metrics`). Expectation (NOT a reported metric, to be confirmed by the run): precision should recover toward ~90% while recall stays ~95.7%. Report only the measured number.
**Blocked by:** None.
